...
use crate::error::{DriftOpsError, ErrorCollector, Result};
...
impl HclParser {
    ...
    pub async fn parse_directory(&self, path: &Path) -> Result<ParsedHcl> {
        if !path.exists() {
            return Err(DriftOpsError::DirectoryNotFound {
    ...
    pub async fn parse_file(&self, path: &Path, repository: Option<&str>) -> Result<ParsedHcl> {
        let content = tokio::fs::read_to_string(path)
            .await
            .map_err(|e| DriftOpsError::io(path, e))?;
...
impl Parser for HclParser {
    fn parse_content(
        ...
        // Parse HCL content
        let body: Body = hcl::from_str(content).map_err(|e| DriftOpsError::HclParse {
...
fn parse_required_version(expr: &Expression, file_path: &Path) -> Result<Constraint> {
    if let Expression::String(version) = expr {
        ...
        return Err(DriftOpsError::HclParse {
            file: file_path.to_path_buf(),
