...
//! driftops scan ./terraform ./modules
//!
...
//! driftops validate driftops.yaml
//! ```
...
/// DriftOps - Terraform/OpenTofu module constraint analyzer and dependency mapper.
#[derive(Parser, Debug)]
#[command(
    name = "driftops",
    author,
    ...
    long_about = "DriftOps scans Terraform/OpenTofu repositories, parses HCL files, builds \
                  dependency graphs, and detects version constraint conflicts, deprecated \
                  modules, and risky patterns.",
    after_help = "For more information, visit: https://github.com/yourusername/driftops"
)]
pub struct Cli {
    /// Path to configuration file
    #[arg(short, long, global = true, env = "DRIFTOPS_CONFIG")]
...
pub struct ScanArgs {
    ...
    #[arg(long, env = "DRIFTOPS_GIT_TOKEN", hide_env_values = true)]
...
pub struct ValidateArgs {
    ...
    #[arg(value_name = "FILE", default_value = "driftops.yaml")]
    pub config: PathBuf,
}
...
mod tests {
    ...
    fn test_scan_command() {
        let cli = Cli::parse_from(["driftops", "scan", "./terraform"]);
        match cli.command {
            Commands::Scan(args) => {
    ...
    fn test_init_command() {
        let cli = Cli::parse_from(["driftops", "init"]);
        assert!(matches!(cli.command, Commands::Init));
    }

    #[test]
    fn test_validate_command() {
        let cli = Cli::parse_from(["driftops", "validate", "custom.yaml"]);
        match cli.command {
    ...
    #[test]
    fn test_alias() {
        let cli = Cli::parse_from(["driftops", "s", "./terraform"]);
