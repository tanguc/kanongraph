...
use crate::error::{DriftOpsError, Result};
...
impl GitClient {
    ...
    pub fn new(config: Config) -> Self {
        ...
        let temp_dir = std::env::temp_dir().join("driftops-repos");
    ...
    pub async fn clone_repository(&self, url: &str) -> Result<PathBuf> {
        ...
        if target_path.exists() {
            ...
            tokio::fs::remove_dir_all(&target_path)
                .await
                .map_err(|e| DriftOpsError::io(&target_path, e))?;
        }
        ...
        tokio::fs::create_dir_all(&self.temp_dir)
            .await
            .map_err(|e| DriftOpsError::io(&self.temp_dir, e))?;
    ...
    fn find_provider(&self, url: &str) -> Result<&Arc<dyn GitProvider>> {
        ...
        Err(DriftOpsError::UnsupportedGitProvider {
            url: url.to_string(),
        })
    }
    ...
    pub fn get_token_for_url(&self, url: &str) -> Result<String> {
        ...
        let platform_str = match platform {
            ...
            ProviderType::Unknown => {
                ...
                return Err(DriftOpsError::UnsupportedGitProvider {
                    url: url.to_string(),
                });
            }
        };
